...
def supervised_train(cascade_level, data_folder_path, alpha_2):
    ...
    while True:
        ...
        folder = r"./data/train_data/supervised/bg_1_958.h5"
        ...
        # New_Locs = get_new_locs(vol_size).to(device) # Compute new coordinate positions
        ...
        for i in range(1, args.n_iter + 1): # 1, args.n_iter + 1
            ...
            if cascade_level == 1:
                ...
                # new_locs_flow = New_Locs(flow_m2f)  # Compute new coordinate positions
                # Calculate loss
        ...
        if sum_MFN < 0:
            ...
            alpha_2 += 0.3
            print(f"sum_MFN > 15,adjust alpha_2 to {alpha_2:.2f},restart training.")
            continue # Same as above, restart again with the new alpha_2
        else:
            # 0 <= sum_MFN <= 15, training ends
            save_file_name = '1_supervised_cascade{}_{}.pth'.format(cascade_level, i)  # First level of the supervised cascade network
            save_file_path = os.path.join(args.supervised_model_dir, save_file_name)
            torch.save(UNet.state_dict(), save_file_path)
            print(f"Training complete, sum_MFN={sum_MFN} is within the [0,15] range; model saved to {save_file_path}", flush=True)
...
def unsupervised_train(cascade_level, data_folder_path, alpha_2):
    ...
    while True:
        ...
        if sum_MFN < 0:
            ...
            save_file_name = '1_unsupervised_cascade{}_{}.pth'.format(cascade_level, i)  # First level of the unsupervised cascade network
