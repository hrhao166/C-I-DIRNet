...
def save_image(img, name, slice_idx=None):
    ...
    img_pil.save(os.path.join(args.result_dir, f"{name}.png"))
...
def train():
    ...
    STN = SpatialTransformer(vol_size).to(device) # 空间变换网络, 用于将形变场施加到浮动图像上, 生成配准后的图像, vol_size = [D, W, H]
    # UNet = UNet.float()  
    UNet.train()
    STN.train()
    # 模型参数个数
    ...
    train_files = glob.glob(os.path.join(args.train_dir, '*.png'))
    ...
    for i in range(1, args.n_iter + 1):
        ...
        tt=end-start
        print("i: %d  loss: %f  sim: %f  grad: %f  train_time: %f"  % (i, loss.item(), sim_loss.item(), grad_loss.item(), tt), flush=True)
